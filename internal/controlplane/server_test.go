...
func TestServer_PublishesOnlyOnChange(t *testing.T) {
	srv := NewServer(nil, nil, nil, nil, nil, nil) // provider unused directly in this test — publishIfChanged doesn't touch it
...
func TestServer_SetOverride_DrainExcludesFromPublishedSetButNotSnapshot(t *testing.T) {
	...
	srv := NewServer(provider, nil, nil, nil, nil, nil)
