...
func TestValidateDataPlane(t *testing.T) {
	tests := []struct {
		...
		{name: "max-conns 0 ok (disabled)", mutate: func(c *DataPlaneConfig) { c.MaxConns = 0 }, wantErr: false},
		{name: "max-conns positive ok", mutate: func(c *DataPlaneConfig) { c.MaxConns = 1000 }, wantErr: false},
		{name: "negative max-conns", mutate: func(c *DataPlaneConfig) { c.MaxConns = -1 }, wantErr: true, substr: "-max-conns"},
		{name: "zero max-header-bytes", mutate: func(c *DataPlaneConfig) { c.MaxHeaderBytes = 0 }, wantErr: true, substr: "-max-header-bytes"},
		{name: "negative max-header-bytes", mutate: func(c *DataPlaneConfig) { c.MaxHeaderBytes = -1 }, wantErr: true, substr: "-max-header-bytes"},
		{name: "max-body-bytes 0 ok (disabled)", mutate: func(c *DataPlaneConfig) { c.MaxBodyBytes = 0 }, wantErr: false},
		{name: "max-body-bytes positive ok", mutate: func(c *DataPlaneConfig) { c.MaxBodyBytes = 1 << 20 }, wantErr: false},
		{name: "negative max-body-bytes", mutate: func(c *DataPlaneConfig) { c.MaxBodyBytes = -1 }, wantErr: true, substr: "-max-body-bytes"},
		{name: "zero read-header-timeout", mutate: func(c *DataPlaneConfig) { c.ReadHeaderTimeout = 0 }, wantErr: true, substr: "-read-header-timeout"},
		{name: "negative read-header-timeout", mutate: func(c *DataPlaneConfig) { c.ReadHeaderTimeout = -1 }, wantErr: true, substr: "-read-header-timeout"},
		{
